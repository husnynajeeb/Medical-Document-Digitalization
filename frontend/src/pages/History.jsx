...
export default function History() {
  ...
  const graphData = reports
    ...
    .map((report, idx) => {
      ...
      const findVal = (key) => {
        const match = report.results.find((x) => x.test.toLowerCase().includes(key));
  ...
  const latestStatus = graphData.length ? graphData[graphData.length - 1].status : null;
  ...
  const CustomXAxisTick = ({ x, y, payload }) => {
    ...
    return (
      <g transform={`translate(${x},${y})`}>
        ...
        <text x={0} y={0} dy={22} textAnchor="middle" fill="#374151" fontSize={14} fontWeight={700}>
          {point.label}
        </text>
        <text x={0} y={0} dy={38} textAnchor="middle" fill="#6b7280" fontSize={12}>
          {point.date}
        </text>
      </g>
    );
  ...
  const CustomDot = (props) => {
    ...
    return (
      <g key={`dot-${payload.idx}`}>
        ...
        <circle cx={cx} cy={cy} r={12} fill="#fff" stroke={color} strokeWidth={3} />
  ...
  const CustomActiveDot = (props) => {
    ...
    return (
      <g key={`adot-${payload.idx}`}>
        ...
        <circle cx={cx} cy={cy} r={16} fill="#fff" stroke={color} strokeWidth={3.5} />
    ...
    return (
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
          ...
          <p style={{ fontWeight: 700, color: "#111827", fontSize: 15, margin: 0 }}>{d.fullLabel}</p>
        </div>
        <p style={{ color: "#9ca3af", fontSize: 12, marginBottom: 10 }}>{d.date}</p>
        <span style={{
        ...
        <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: 8 }}>
          <p style={{ color: "#374151", marginBottom: 3 }}>Total tests: <b>{d.totalTests}</b></p>
        ...
        {graphData.length > 0 && (
          <div className="grid grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-2xl shadow p-5 flex flex-col gap-1 border-t-4 border-blue-500">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Total Reports</p>
              <p className="text-3xl font-bold text-gray-800">{graphData.length}</p>
            </div>
            {/* Normal */}
            <div className="bg-white rounded-2xl shadow p-5 flex flex-col gap-1 border-t-4 border-green-500">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Normal</p>
            ...
            <div className="bg-white rounded-2xl shadow p-5 flex flex-col gap-1 border-t-4 border-amber-400">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Partial</p>
            ...
            <div className="bg-white rounded-2xl shadow p-5 flex flex-col gap-1 border-t-4 border-red-500">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Critical</p>
          ...
          <div className="p-8">
            ...
            <div className="flex items-start justify-between mb-8">
              ...
              <div className="flex gap-5">
                {[["Normal","#16A34A"],["Partial","#D97706"],["Critical","#DC2626"]].map(([label, color]) => (
              ...
              <ResponsiveContainer width="100%" height={graphHeight}>
                ...
                >
                  <defs>
                    ...
                    <linearGradient id="partialZone" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <YAxis
                    ticks={[0, 1, 2]}
                    tickFormatter={(v) => v === 0 ? "Normal" : v === 1 ? "Partial" : "Critical"}
            ...
            {graphData.length >= 2 && (() => {
              ...
              return (
                <div style={{
                  ...
                  alignItems: "center",
                  gap: 8,
                  background: same ? "#f8fafc" : improved ? "#f0fdf4" : "#fef2f2",
                  border: `1px solid ${same ? "#e2e8f0" : improved ? "#bbf7d0" : "#fecaca"}`,
                  borderRadius: 24,
                  padding: "6px 16px",
                  fontSize: 13,
                  fontWeight: 600,
                  color: same ? "#64748b" : improved ? "#16a34a" : "#dc2626",
                }}>
                  <span style={{ fontSize: 16 }}>{same ? "→" : improved ? "↓" : "↑"}</span>
                  {same
